...
| Decisión | Opciones | Evidencia (nivel) | Estado y veredicto |
|---|---|---|---|
| Comparar recuperación densa, BM25 e híbrida | tres estrategias | N1, el experimento central: métrica de recuperación con intervalo de confianza por bootstrap, contraste pareado frente a la densa y desglose por tipo de pregunta; ajustado en desarrollo y reportado en el conjunto de prueba reservado | [Cerrada] (prueba, n=28, report `retrieval_20260625T111234Z`). La densa gana con 0.797; la fusión convexa con peso alto al denso queda en 0.769 (diferencia de −0.028, no significativa); la fusión por rango (RRF) en 0.706 (−0.091, significativamente peor); BM25 sola en 0.507 (significativamente peor). El sistema usa recuperación densa |
| Elegir la función de fusión: por rango (RRF) o convexa por puntuación normalizada | RRF / convexa / peso α | N1 | [Cerrada] En desarrollo y prueba, la convexa con peso alto al denso supera a la fusión por rango; el RRF es significativamente peor que la densa sola. La mejor fusión solo empata con la densa, no la mejora |
...
Conviene ser claro con los límites: la muestra es de 53 preguntas (desarrollo), y la significación viene del contraste pareado pese a que los intervalos marginales se solapen; la confirmación final se hizo en el conjunto de prueba reservado. Además, la comparativa se hizo sobre la vista de fragmento con contexto; comparar esa vista con la del texto crudo en el modelo ganador queda pendiente.
...
La pregunta era cuánto contexto pasar al generador y cómo. Se midió la cobertura de la evidencia y una métrica propia de señal frente a ruido dentro del contexto. La expansión completa quedó descartada (igual cobertura, la mitad de densidad y el doble de texto), y pasar solo el fragmento perdía cobertura. Más de cinco fragmentos era pura dilución. La confirmación decisiva la dio la propia generación: con tres fragmentos las respuestas fueron mejores que con cinco, tanto en hechos clave recuperados (0.74 frente a 0.57) como en calidad de las citas (0.88 frente a 0.67). Es decir, más contexto empeora la respuesta, un efecto conocido cuando la información relevante queda "perdida en medio" de mucho texto. La configuración quedó fijada en expansión acotada, 4000 caracteres y tres fragmentos.
