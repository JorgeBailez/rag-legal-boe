...
def is_form_b_table(table: etree._Element) -> bool:
    """True si la tabla es «forma B»: texto CRUDO en `<td>`/`<th>` SIN `<p>` dentro.

    El BOE representa tablas de dos formas. Forma A: `<td><p class="cuerpo_tabla_*">texto</p></td>`
    — la capta `version.iter("p")`. Forma B: `<td>texto</td>` directo, sin `<p>` — el cuerpo
    `<p>`-céntrico la **omitía en silencio** (ley vigente perdida). Esta función la detecta para
    linealizarla aparte; si la tabla trae algún `<p>`, es forma A y se trata por la vía de `<p>`.
    """
    if table.find(".//p") is not None:
        return False
    return any(_element_text(c) for c in table.iter("td", "th"))


def _linearize_table_rows(table: etree._Element) -> list[tuple[str, str]]:
    """Linealiza una tabla forma B POR FILA: une las celdas no vacías de cada `<tr>` en una línea.
...
def classify_version_paragraphs(
    ...
    Recorre la versión en **orden de documento** capturando, además de los `<p>`, las **tablas
    forma B** (celdas `<td>`/`<th>` con texto crudo sin `<p>`, que el cuerpo `<p>`-céntrico perdía).
    Las tablas forma A (`<td><p class="cuerpo_tabla">`) ya las capta la vía de `<p>` y NO se
    duplican. La regla de blockquote se respeta también para las tablas (forma B dentro de
    blockquote = editorial → `dropped`; fuera = vigente → `kept`).

    Devuelve `(kept, notes, dropped)`:
    - `kept`: cuerpo normativo indexable (`<p>` + filas de tabla forma B vigente).
    ...
    notes: list[dict] = []
    dropped: list[dict] = []
    form_b_tables = {t for t in version.iter("table") if is_form_b_table(t)}
    for el in version.iter():
        ...
        elif el.tag == "table" and el in form_b_tables:
            # Tabla forma B en su posición de documento: filas al cuerpo (o a dropped si es
            # editorial en blockquote). Sus <tr>/<td>/<th> se ignoran luego (no duplican).
            in_blockquote = next(el.iterancestors("blockquote"), None) is not None
