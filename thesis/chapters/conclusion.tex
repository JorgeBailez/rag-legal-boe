...
\begin{description}
  ...
  \item[OE-03 (estrategias de recuperación).] \textbf{Conseguido con un matiz.} Se implementaron y evaluaron las recuperaciones léxica (BM25 con analizador de español), semántica (índice denso exacto) e híbrida (fusión por rangos recíprocos y combinación convexa). La \emph{reordenación} (\emph{reranking}), contemplada en el enunciado del objetivo, no se implementó: se justificó como trabajo futuro con nivel de evidencia de literatura, al no esperarse mejora fiable sobre un denso fuerte en este corpus.
...
\begin{itemize}
  ...
  \item \textbf{Reducir la sobre-abstención sin comprometer la seguridad.} Aprovechar la excelente separabilidad de la señal de recuperación para un umbral de confianza, o ensayar \emph{prompts} menos conservadores, midiendo el efecto sobre las respuestas indebidas.
