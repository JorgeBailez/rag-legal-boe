...
El capítulo anterior ha definido el objetivo general del Trabajo Fin de Grado: diseñar, implementar y evaluar una aplicación web local basada en una arquitectura de generación aumentada mediante recuperación para facilitar la consulta ciudadana de normativa consolidada. Para abordar ese objetivo no basta con seleccionar un modelo generativo y formular preguntas sobre los documentos. El sistema debe localizar información relevante, conservar la relación entre cada fragmento y su fuente normativa, construir un contexto útil para la generación, producir una respuesta comprensible y permitir que el usuario revise las evidencias utilizadas.

Este capítulo revisa los fundamentos y trabajos relacionados necesarios para justificar ese enfoque. En primer lugar, se analizan las particularidades de la información normativa consolidada y su efecto sobre el diseño de sistemas de consulta. A continuación se presentan las principales familias de recuperación de información: recuperación léxica, recuperación semántica y recuperación híbrida. Después se introducen los modelos de lenguaje, sus limitaciones y el papel de la generación aumentada mediante recuperación, conocida habitualmente como \emph{Retrieval-Augmented Generation} (RAG). Finalmente, se revisan criterios de evaluación y se sitúa el proyecto dentro del dominio jurídico.

La finalidad del capítulo no es describir todavía la arquitectura concreta desarrollada. Las decisiones específicas del sistema, como la estructura de los datos, la estrategia de fragmentación, los modelos seleccionados o los parámetros experimentales, se presentan en los capítulos de diseño, implementación y validación. Aquí se exponen los conceptos y alternativas existentes que permiten comprender esas decisiones.

\section{Información jurídica y normativa consolidada}
\label{sec:informacion-juridica}

Los documentos jurídicos presentan características que los diferencian de otras colecciones textuales utilizadas habitualmente en sistemas de recuperación de información. Una norma no es solo una secuencia de párrafos: posee una estructura interna formada por títulos, capítulos, artículos, disposiciones, anexos y otros elementos que condicionan su interpretación. Además, cada documento se inserta en un contexto temporal y normativo determinado, por lo que resulta necesario conocer su versión, su vigencia y su procedencia.
...
Otra dificultad específica del dominio normativo es la diferencia entre el lenguaje de la ciudadanía y la redacción formal de las normas. Una persona puede preguntar, por ejemplo, ``¿cuántos días tengo para recurrir una multa?'' o ``¿pueden despedirme durante una baja médica?'', mientras que los documentos normativos pueden emplear expresiones como ``plazo para interponer recurso'', ``procedimiento sancionador'', ``extinción del contrato de trabajo'' o ``incapacidad temporal''. Esta distancia lingüística justifica la necesidad de estudiar técnicas de recuperación que no dependan únicamente de coincidencias exactas entre las palabras de la consulta y las palabras del documento.
...
El concepto de relevancia es central. Un fragmento puede ser relevante porque contiene directamente la respuesta, porque aporta una definición necesaria, porque delimita una excepción o porque enlaza con otra parte de la norma. En sistemas RAG, la recuperación adquiere una importancia adicional: los documentos seleccionados no son el resultado final, sino la información que condicionará la respuesta generada por el modelo de lenguaje. Si el recuperador omite un fragmento esencial, la fase generativa puede responder de forma incompleta; si incluye demasiado ruido, el modelo puede apoyarse en información irrelevante o contradictoria.

Los sistemas de recuperación suelen ordenar los resultados y devolver únicamente los primeros elementos, habitualmente denominados \emph{top-k}. Esta selección es necesaria por razones prácticas: los modelos generativos tienen una ventana de contexto limitada y no pueden recibir todo el corpus en cada consulta. Por tanto, el recuperador actúa como un filtro que determina qué partes del corpus estarán disponibles para la generación.

La evaluación de la recuperación se apoya en métricas que comparan los documentos recuperados con un conjunto de referencias consideradas relevantes. Entre las más habituales se encuentran la precisión, la exhaustividad, \emph{Recall@k}, \emph{Mean Reciprocal Rank} (MRR) y \emph{Normalized Discounted Cumulative Gain} (nDCG). Cada una mide un aspecto diferente: si los resultados son relevantes, si aparecen suficientes evidencias, si la primera respuesta útil aparece pronto o si el orden de los documentos recuperados es adecuado. En el capítulo de experimentos se concretará qué métricas se utilizan y cómo se aplican al corpus del proyecto.

El benchmark BEIR ilustra la diversidad de enfoques de recuperación y la necesidad de evaluar en condiciones heterogéneas. Sus autores comparan modelos léxicos, dispersos, densos, de interacción tardía y de reordenación sobre múltiples conjuntos de datos, y muestran que BM25 continúa siendo una línea base robusta, mientras que algunas técnicas de reordenación pueden mejorar resultados a costa de un mayor coste computacional \cite{thakur2021beir}. Esta observación es relevante para el presente TFG: la recuperación semántica puede resultar atractiva por su capacidad para capturar reformulaciones, pero debe compararse empíricamente con alternativas léxicas e híbridas.

\section{Recuperación léxica}
\label{sec:recuperacion-lexica}

La recuperación léxica se basa en la coincidencia entre los términos de la consulta y los términos presentes en los documentos. Su enfoque parte de una intuición sencilla: un documento que contiene palabras relevantes de la consulta tiene más probabilidad de responder a la necesidad de información. Para que esta búsqueda sea eficiente, los sistemas suelen utilizar índices invertidos, estructuras que asocian cada término con los documentos o fragmentos en los que aparece.

Uno de los modelos más utilizados en recuperación léxica es BM25. Este método procede del marco probabilístico de relevancia y combina varias señales: la presencia de términos de la consulta, la frecuencia con la que aparecen en un documento, la rareza de esos términos dentro de la colección y una normalización que evita favorecer de forma excesiva a los documentos más largos \cite{robertson2009bm25}. En términos prácticos, BM25 asigna una puntuación más alta a los documentos que contienen términos informativos de la consulta de manera significativa, pero controla el efecto de repeticiones excesivas y de diferencias de longitud.
...
No obstante, la recuperación léxica también presenta limitaciones. Su rendimiento depende del solapamiento superficial entre consulta y documento. Si una persona pregunta con vocabulario cotidiano y la norma utiliza términos técnicos diferentes, el sistema puede no recuperar el pasaje relevante. Por ejemplo, una consulta como ``¿qué pasa si la administración no me contesta?'' puede estar relacionada con el concepto jurídico de ``silencio administrativo'', aunque no contenga esa expresión. Este tipo de diferencia entre lenguaje ciudadano y lenguaje normativo justifica estudiar técnicas semánticas.

En este TFG, la recuperación léxica no se tratará como una solución obsoleta ni como un mero trámite. Se considera una línea base necesaria y una estrategia potencialmente útil, especialmente en consultas con términos jurídicos precisos. Su rendimiento deberá compararse con el de la recuperación semántica y la recuperación híbrida en el capítulo experimental.

\section{Representaciones vectoriales y recuperación semántica}
...
Cuando el corpus crece, la búsqueda exhaustiva sobre todos los vectores puede resultar costosa. Por ello suelen emplearse índices vectoriales o bases de datos vectoriales, diseñadas para almacenar embeddings y realizar búsquedas eficientes de similitud. Estas herramientas pueden incluir técnicas de búsqueda aproximada, filtrado por metadatos y persistencia de índices.
...
En este TFG, la recuperación híbrida se considera una estrategia que debe implementarse y evaluarse, pero el método concreto de combinación se decidirá en el capítulo de diseño y se justificará mediante experimentos. Esta separación es importante: el estado del arte describe alternativas; la implementación selecciona una o varias de ellas; y la validación determina su rendimiento real en el corpus normativo utilizado.

\section{Modelos de lenguaje}
\label{sec:modelos-lenguaje}

Los modelos de lenguaje son sistemas capaces de estimar y generar secuencias de texto a partir de patrones aprendidos durante su entrenamiento. En las arquitecturas modernas, estos modelos suelen estar basados en Transformer, una arquitectura introducida por Vaswani et al. que sustituyó las estructuras recurrentes tradicionales por mecanismos de atención \cite{vaswani2017attention}. La atención permite que el modelo relacione diferentes posiciones de una secuencia y utilice información contextual para producir representaciones más ricas.

Para comprender su papel en un sistema RAG no es necesario desarrollar toda la arquitectura interna del Transformer, pero sí conviene introducir algunos conceptos. En primer lugar, los modelos de lenguaje operan sobre unidades llamadas \emph{tokens}, que pueden corresponder a palabras, partes de palabras o signos. En segundo lugar, generan texto de forma condicionada: la salida depende del texto de entrada, de las instrucciones proporcionadas y del contexto disponible. En tercer lugar, cada modelo tiene una ventana de contexto limitada, lo que impide introducir documentos extensos completos en cada consulta.
...
Un flujo RAG típico consta de varias etapas. Primero, el sistema recibe una pregunta. Después, un recuperador selecciona documentos o fragmentos relevantes de la colección. A continuación, esos fragmentos se ensamblan en un contexto que se entrega al modelo de lenguaje junto con la pregunta y las instrucciones. Finalmente, el modelo genera una respuesta que debería estar fundamentada en el contexto suministrado.
...
La construcción de contexto es la etapa posterior a la recuperación. Su objetivo es transformar los fragmentos recuperados en una entrada útil para el modelo generativo. Esta etapa puede incluir deduplicación, selección de las evidencias mejor posicionadas, expansión con contexto cercano, ordenación, limitación por presupuesto de tokens y asignación de identificadores de evidencia. En el dominio jurídico, esta etapa debe preservar la procedencia de cada fragmento para que la respuesta pueda citar evidencias revisables.
...
Muchas de estas dimensiones, especialmente la fidelidad y la corrección, son difíciles de medir con métricas automáticas sencillas porque exigen «entender» el contenido de la respuesta. Una alternativa cada vez más extendida consiste en emplear un modelo de lenguaje potente como \emph{juez} (\emph{LLM-as-judge}) que valore la salida de otro modelo \cite{zheng2023judge}. Este enfoque escala la evaluación y se correlaciona razonablemente con el juicio humano, pero arrastra sesgos documentados: el juez puede verse influido por la posición de las respuestas, por su longitud o por una preferencia hacia textos de su propio estilo (sesgo de auto-preferencia) \cite{zheng2023judge}. De ahí dos cautelas habituales: utilizar como juez un modelo de familia distinta a la del generador, para mitigar la auto-preferencia, y validar sus veredictos frente a anotación humana —por ejemplo, midiendo el acuerdo con el coeficiente $\kappa$ de Cohen— antes de confiar en ellos. Sin esa validación, las puntuaciones de fidelidad y corrección deben considerarse provisionales.

Estas propuestas son útiles como marco conceptual, pero no sustituyen necesariamente a una evaluación diseñada para el dominio concreto. En este TFG, la evaluación deberá combinar métricas reproducibles de recuperación, revisión cualitativa de respuestas, comprobación de citas y consultas fuera del corpus. Sobre todo, deberá ser \textbf{modular}: medir cada componente por separado de modo que un fallo pueda atribuirse a su causa real —la recuperación, el ensamblado de contexto, la fidelidad a las evidencias, las citas, la corrección o la abstención— y no quedarse en una nota global que no indica dónde mejorar. La categoría de preguntas fuera del corpus es especialmente importante, porque el sistema debe reconocer que no dispone de información suficiente cuando la pregunta no puede responderse con las evidencias disponibles.
...
La comparación entre recuperación léxica, semántica e híbrida es una parte central del trabajo porque cada estrategia responde a una hipótesis distinta. La recuperación léxica puede funcionar bien cuando la consulta contiene términos jurídicos específicos. La recuperación semántica puede aportar valor cuando el usuario formula la pregunta con lenguaje cotidiano. La recuperación híbrida puede combinar ambas señales, aunque su superioridad debe demostrarse empíricamente. El \emph{reranking} mediante modelos neuronales, como la reordenación de pasajes con BERT \cite{nogueira2019bert}, se considera una alternativa relevante, aunque su incorporación queda condicionada al coste computacional y a la mejora observada.
