...
Existe además un paquete \texttt{app} reservado a una futura interfaz web o API de servicio; queda fuera del alcance de esta entrega, en coherencia con los objetivos (Capítulo~\ref{chap:objetivos}), donde la interfaz de usuario se plantea como trabajo futuro y el sistema se demuestra mediante línea de comandos y cuadernos reproducibles.
...
Algunas exclusiones merecen mención porque ilustran los criterios en acción. Se descartaron normas derogadas que un sistema ingenuo podría haber incluido por inercia. La Ley 35/2015 quedó fuera porque no posee texto consolidado propio —es una ley de reforma cuyo contenido vive en el anexo de otra norma— y la API devolvía un error para ella. Y dos textos de gran tamaño y formato antiguo, el Código Civil y la Ley Orgánica del Poder Judicial, se difirieron de forma consciente por su riesgo para el \emph{parser} y su peso desproporcionado en el índice, una decisión que prioriza la fiabilidad del conjunto sobre la mera acumulación de volumen.

\subsection{Construcción, validación y auditoría}
\label{subsec:construccion-validacion}

Sobre ese catálogo operan varios procesos con responsabilidades separadas, que reflejan en el plano operativo los principios de diseño enunciados antes. La construcción \emph{con red} recorre el corpus semilla y, para cada norma, descarga su \emph{raw} y su manifiesto, la verifica contra los criterios de inclusión y, si los cumple, genera ya su documento procesado y sus \emph{chunks}; las normas que no los cumplen se dejan fuera y se documentan en un informe de verificación. A partir de ahí, todas las operaciones habituales son \emph{sin red}: un reprocesado local que regenera los artefactos procesados a partir del \emph{raw} ya descargado —útil cada vez que mejora el \emph{parser} o el fragmentador, sin volver a molestar a la API—, una validación estructural que comprueba que cada documento y cada conjunto de \emph{chunks} cumplen sus contratos, y una auditoría de calidad, de solo lectura, que revisa la coherencia relacional del corpus y produce un informe de \emph{readiness} que actúa como puerta previa a la indexación.
